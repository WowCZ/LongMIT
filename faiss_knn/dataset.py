...
def create_dataset_from_oivf_config(cfg, ds_name):
    ...
    return MultiFileVectorDataset(
        ...
        [
            FileDescriptor(
                f["name"], f["format"], np.dtype(f["dtype"]), f["size"], f["domain"], f["embed_model"]
            )
...
class FileDescriptor:
    def __init__(self, name: str, format: str, dtype: np.dtype, size: int, domain: str, embed_model: str):
        self.name = name
        self.format = format
        self.dtype = dtype
        self.size = size
...
class MultiFileVectorDataset:
    def __init__(
        ...
        for f in self.file_descriptors:
            xb = _memmap_vecs(
                f"{self.root}/{f.domain}/{f.embed_model}/{f.name}", f.format, f.dtype, f.size, self.d
            )
            t += xb.shape[0]
    ...
    def iterate(self, start: int, batch_size: int, dt: np.dtype):
        ...
        for f in self.file_descriptors:
            ...
            xb = _memmap_vecs(
                f"{self.root}/{f.domain}/{f.embed_model}/{f.name}",
                f.format,
                f.dtype,
                f.size,
                self.d,
